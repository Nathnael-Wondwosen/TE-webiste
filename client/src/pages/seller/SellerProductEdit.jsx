...
const SellerProductEdit = () => {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      try {
        ...
        // Handle category based on the data format
        let categoryValue = '';
        if (typeof data.category === 'string') {
          ...
          if (data.category._id) {
            ...
            categoryValue = data.category._id;
          } else if (data.category.name) {
            // If it has a name property, we'll try to match it later
            // after categories are loaded
            categoryValue = data.category.name;
          }
        }
        ...
        setForm({
          ...
          productType: data.productType || 'Local',
        });
        const existing = (data.images || []).map((image) => ({
        ...
        setImages(existing);
        const videos = (data.videos || []).map((video) => video.url).filter(Boolean);
        setVideoLinks(videos.length ? videos : ['']);
      } catch (error) {
  ...
  // After categories are loaded, check if we need to convert category name to ID
  useEffect(() => {
    if (categories.length > 0 && form.category && !categories.some(cat => cat._id === form.category)) {
      // The current category value is not a valid ID, try to find it by name
      const categoryByName = categories.find(cat => cat.name === form.category);
      if (categoryByName) {
        setForm(prev => ({
          ...prev,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setMessage('Unable to update product right now.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="bg-white rounded-3xl border border-slate-200/70 p-6 shadow-sm">
        ...
        <h1 className="text-2xl font-semibold text-slate-900 mt-2">Edit product</h1>
        <p className="text-sm text-slate-500 mt-2">
          Update product details and manage images. The first image remains primary.
        </p>
      </div>
